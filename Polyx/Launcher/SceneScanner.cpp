...
std::vector<SceneInfo> ScanScenes()
{
    ...
    std::cout << "Scanning scenes in: "
              << fs::absolute(scenesPath)
              << std::endl;

    if (!fs::exists(scenesPath) ||
        !fs::is_directory(scenesPath))
    {
        std::cout << "Scenes folder not found!"
                  << std::endl;
    ...
    for (const auto& entry : fs::directory_iterator(scenesPath))
    {
        ...
        // --------------------------------------------------
        // ВАЖНО:
        // Обрабатываем ТОЛЬКО файлы сцен
        // --------------------------------------------------
        ...
        fs::path jpgPreview =
            ...
            (scene.name + "_preview.jpg");

        fs::path pngPreview =
            entry.path().parent_path() /
            (scene.name + "_preview.png");
        ...
        try
        {
            ...
            if (!scene.creationDate.empty() &&
                scene.creationDate.back() == '\n')
            {
